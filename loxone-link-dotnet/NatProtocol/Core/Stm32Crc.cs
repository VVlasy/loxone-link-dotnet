...
namespace loxonelinkdotnet.NatProtocol.Core
{
     ...
     * STM32 peripheral CRC == CRC32_MPEG2 & Big Endian
     */

    public sealed class Stm32Crc : HashAlgorithm
    {
        ...
        public Stm32Crc(uint polynomial, uint seed)
        {
        ...
        private static uint CalculateHash(uint[] table, uint seed, byte[] buffer, int start, int size)
        {
            ...
            for (var i = start; i < size - start; i+=4)
            {
                var pos = (byte) ((crc ^ buffer[i + 3] << 24) >> 24);
                crc = crc << 8 ^ table[pos];

                pos = (byte) ((crc ^ buffer[i + 2] << 24) >> 24);
                crc = crc << 8 ^ table[pos];

                pos = (byte) ((crc ^ buffer[i + 1] << 24) >> 24);
                crc = crc << 8 ^ table[pos];

                pos = (byte) ((crc ^ buffer[i] << 24) >> 24);
                crc = crc << 8 ^ table[pos];
            }
